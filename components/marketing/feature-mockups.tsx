 ...
 * dashboard card's shell (rounded-2xl, border-zinc-200, soft shadow, header row
 * with title + status pill) so they read as one family. Visual only, not
 * functional (no real logic) and intentionally lightweight (CSS/divs, no
 * images) to keep mobile performance high.
 */
function MockCard({
  title,
  badge,
  ...
  return (
    <div className={cn("overflow-hidden rounded-2xl border border-zinc-200 bg-white shadow-xl shadow-zinc-900/5", className)}>
      <div className="flex items-center justify-between border-b border-zinc-100 px-4 py-3">
        ...
        {badge && (
          <span className={cn("rounded-full px-2 py-0.5 text-[11px] font-semibold", tone)}>{badge}</span>
        )}
      </div>
...
// Per-day slots: a tint key drives the block color; empty slots are neutral.
  ...
  return (
    <MockCard title="Mailroom" badge="3 new">
      <div className="space-y-2">
        {rows.map((r) => (
          <div key={r.who} className="flex items-center gap-3 rounded-xl border border-zinc-100 p-2.5">
        ...
        <div className="flex items-center gap-3 rounded-xl border border-amber-200 bg-amber-50/60 p-2.5">
    ...
    <MockCard title="Tax invoice" badge="ZATCA">
      ...
      <dl className="mt-3 space-y-2 text-xs">
        ...
        <div className="flex justify-between border-t border-zinc-100 pt-2 text-sm">
          ...
          <dd className="font-bold text-emerald-700">AED 1,050.00</dd>
        </div>
      </dl>
    </MockCard>
  );
...
export function WhatsAppMock() {
  return (
    <MockCard title="Member chat" badge="Online">
      <div className="space-y-2">
        <div className="max-w-[80%] rounded-2xl rounded-tl-sm bg-zinc-100 px-3 py-2 text-xs text-zinc-700">
          Is a hot desk free tomorrow morning?
        </div>
        <div className="ml-auto max-w-[80%] rounded-2xl rounded-tr-sm bg-emerald-600 px-3 py-2 text-xs text-white">
          Yes, 9am is open. Want me to book it?
        </div>
        <div className="ml-auto flex max-w-[85%] items-center gap-2 rounded-2xl rounded-tr-sm border border-emerald-100 bg-emerald-50 px-3 py-2">
